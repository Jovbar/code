...
class BasicUnit(nn.Module):
    def __init__(self, channels: int, dropout: float):
        ...
        self.block = nn.Sequential(OrderedDict([
            ...
            ("1_activation", nn.ReLU(inplace=True)),#inplace=true,表示x'会使用x的内存空间
...
class WideResNet(nn.Module):
    def __init__(self, depth: int, width_factor: int, dropout: float, in_channels: int, labels: int):
        ...
        self.f = nn.Sequential(OrderedDict([
            ...
            ("7_flattening", nn.Flatten()),#打散
            ("8_classification", nn.Linear(in_features=self.filters[3]*49, out_features=labels)),
        ]))
        
        self._initialize()

    def _initialize(self):#权值参数初始化
